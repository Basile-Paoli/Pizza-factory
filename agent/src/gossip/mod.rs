...
pub fn start_gossip(
    ...
    let shared_state: SharedGossipState =
        Arc::new(RwLock::new(GossipState::new(addr, local_skills)));

    let socket = UdpSocket::bind(addr)?;
    let (command_sender, command_receiver) = std::sync::mpsc::channel::<Command>();

    start_gossip_loop(shared_state.clone(), socket.try_clone()?);
    start_listener(socket.try_clone()?, shared_state.clone());
